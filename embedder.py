...
class Reranker:
    ...
    def rerank(self, query: str,
        ...
        混合评分:
          cross_score * 0.6 + feedback_score * 0.2 + recency_score * 0.1 + layer_score * 0.1

        延迟监控: cross-encoder >100ms 时降级到 score_only 模式。
        """
        import time
        ...
        for r in results:
            ...
            # Feedback score: 正反馈多、负反馈少
            pf = r.get("positive_feedback", 0) or 0
            nf = r.get("negative_feedback", 0) or 0
            feedback_score = min(1.0, max(0.0, 0.5 + (pf - nf * 2) * 0.05))

            # Recency score: 最近访问越近越高
            ...
            if last_accessed:
                try:
                    ...
                    recency_score = max(0.1, min(1.0, 1.0 / (1.0 + age_hours / 24)))
                except Exception:
                    pass

            # Layer score
            layer = r.get("layer", "")
            layer_score = {"consolidated": 1.0, "candidate": 0.6, "instant": 0.3}.get(layer, 0.3)

            # Store component scores for later fusion
            r["_base_score"] = score
            r["_feedback_score"] = feedback_score
            r["_recency_score"] = recency_score
            r["_layer_score"] = layer_score

        # 2. Cross-encoder 精确重排 (若可用, 带延迟监控)
        ...
        if self._model and self._model != "score_only":
            try:
                ...
                cross_latency_ms = (time.monotonic() - t0) * 1000

                # Latency degradation: >100ms → fall back to score_only
                if cross_latency_ms > 100:
                    logger.info("Reranker: cross-encoder slow (%.0fms), degrading to score_only", cross_latency_ms)
                    self._model = "score_only"
                else:
                    for i, cs in enumerate(cross_scores):
                        if i < len(results):
                            r = results[i]
                            # Mixed scoring: cross * 0.6 + feedback * 0.2 + recency * 0.1 + layer * 0.1
                            cross_norm = max(0.0, min(1.0, float(cs)))
                            r["rerank_score"] = round(
                                cross_norm * 0.6
                                + r["_feedback_score"] * 0.2
        ...
        # 3. Score-only fallback (no cross-encoder or degraded)
        for r in results:
        ...
        for r in results:
            r.pop("_base_score", None)
            r.pop("_feedback_score", None)
            r.pop("_recency_score", None)
            r.pop("_layer_score", None)
