...
# ═══════════════════════════════════════════════════════════
# Git-based Team Memory Sharing (P2-10)
# ═══════════════════════════════════════════════════════════
...
def _get_memory_dir() -> Path:
    """Directory for team-shared memory files."""
    d = Path.home() / ".hermes" / "data" / "nexus" / "shared"
    d.mkdir(parents=True, exist_ok=True)
    return d


def git_sync_init(remote_url: str = "") -> Dict[str, Any]:
    """Initialize Git-based memory sync.

    Args:
        remote_url: Optional Git remote URL. If empty, uses local-only Git repo.
    """
    mem_dir = _get_memory_dir()
    git_dir = mem_dir / ".git"

    if not git_dir.exists():
        subprocess.run(["git", "init"], cwd=str(mem_dir), capture_output=True)
        # .gitattributes for markdown diffs
        (mem_dir / ".gitattributes").write_text("*.md diff=markdown\n")
        # .gitignore
        (mem_dir / ".gitignore").write_text("*.pyc\n__pycache__/\n*.db\n")
        subprocess.run(["git", "add", "."], cwd=str(mem_dir), capture_output=True)
        subprocess.run(
            ["git", "commit", "-m", "nexus: init shared memory"],
            cwd=str(mem_dir), capture_output=True,
        )
    ...
    if remote_url:
        ...
        result = subprocess.run(
            ...
            cwd=str(mem_dir), capture_output=True, text=True,
        )
    ...
    return {"status": "ok", "path": str(mem_dir), "remote": remote_url or "local"}


def git_sync_push(message: str = "nexus: auto sync") -> Dict[str, Any]:
    """Commit and push shared memories to remote."""
    mem_dir = _get_memory_dir()
    try:
        subprocess.run(["git", "add", "-A"], cwd=str(mem_dir), capture_output=True)
        subprocess.run(
            ["git", "commit", "-m", message],
            cwd=str(mem_dir), capture_output=True,
        )
        result = subprocess.run(
            ["git", "push"], cwd=str(mem_dir), capture_output=True, text=True,
        )
        return {"status": "ok" if result.returncode == 0 else "error",
                "detail": result.stderr or "pushed"}
    except Exception as e:
        return {"status": "error", "detail": str(e)}


def git_sync_pull() -> Dict[str, Any]:
    """Pull shared memories from remote."""
    mem_dir = _get_memory_dir()
    try:
        result = subprocess.run(
            ["git", "pull", "--rebase"], cwd=str(mem_dir),
...
def git_sync_status() -> Dict[str, Any]:
    """Check sync status: ahead/behind counts, changed files."""
    mem_dir = _get_memory_dir()
    try:
        # Changed files
        result = subprocess.run(
