...
def freshness_label(valid_from, valid_to=None) -> str:
    """人类可读的新鲜度标签。"""
    now = datetime.now(timezone.utc)
    if valid_to:
        try:
            vt = datetime.fromisoformat(valid_to) if isinstance(valid_to, str) else valid_to
...
def freshness_score(valid_from, valid_to=None) -> float:
    """新鲜度分数 (0-1)。30 天线性衰减到 0，过期直接返回 0。"""
    now = datetime.now(timezone.utc)
    if valid_to:
        try:
            vt = datetime.fromisoformat(valid_to) if isinstance(valid_to, str) else valid_to
    ...
    try:
        ...
        return 0.3  # unknown time gets low freshness
    days = (now - vf).days
    return max(0.0, 1.0 - days / 30)


def access_score(last_accessed, access_count: int = 0) -> float:
    """访问频率衰减分数 (0-1)。

    基于指数衰减: 0天=1.0, 7天≈0.70, 30天≈0.21, 90天≈0.01。
    额外加成: 访问次数越多基础分越高 (log 衰减)。
    """
    now = datetime.now(timezone.utc)
    try:
        la = datetime.fromisoformat(last_accessed) if isinstance(last_accessed, str) else last_accessed
    ...
    days = (now - la).days
    decay = 0.95 ** days
    # log frequency bonus: 0次=0, 1次=0, 10次≈0.5, 100次≈1.0
    import math
    freq_bonus = min(1.0, math.log1p(access_count) / math.log1p(100)) * 0.3
    return min(1.0, decay * 0.7 + freq_bonus)


_TYPE_WEIGHTS = {"user": 1.0, "feedback": 0.8, "project": 0.6, "reference": 0.4}


def retrieval_priority(memory: Dict[str, Any]) -> float:
    """综合检索排序优先级 (0-1)，越高越优先。

    权重分配:
      40% quality  — 6 域评分均值 (domain_scores)
      25% freshness — 新鲜度
      15% access   — 访问频率衰减
      10% confidence — 置信度
      10% type     — 知识类型权重
    """
    # quality: domain_scores 均值
    ds = memory.get("domain_scores")
    ...
    if ds and isinstance(ds, dict):
        ...
        quality = sum(vals) / len(vals) if vals else 0.5
    else:
        quality = 0.5

    # freshness
    vf = memory.get("valid_from") or memory.get("created_at")
    vt = memory.get("valid_to")
    f_score = freshness_score(vf, vt) if vf else 0.5

    # access
    la = memory.get("last_accessed") or memory.get("created_at")
    ac = memory.get("access_count") or 0
    a_score = access_score(la, ac)

    # confidence
    conf = memory.get("confidence")
    if conf is None:
        # fallback: try belief table
        kb = memory.get("_belief_confidence")
        conf = kb if kb is not None else 0.8
    conf = max(0.0, min(1.0, float(conf)))

    # type weight
    mem_type = memory.get("type", memory.get("layer", "reference"))
    tw = _TYPE_WEIGHTS.get(mem_type, 0.5)
