...
_INTENT_WEIGHTS = {
    "exact":      {"fts": 0.70, "vec": 0.15, "graph": 0.15},
    "semantic":   {"fts": 0.15, "vec": 0.65, "graph": 0.20},
    "relational": {"fts": 0.10, "vec": 0.15, "graph": 0.75},
    "auto":       {"fts": 0.40, "vec": 0.35, "graph": 0.25},
}

_EXACT_PATTERNS = re.compile(
    r'\b\d{3,}\b|error|code|errno|exception|traceback|stack|log|'
    r'function|class|method|variable|file|path|config|version|'
    r'函数|方法|类|变量|参数|错误|异常|日志|配置|版本', re.IGNORECASE
)
_RELATIONAL_PATTERNS = re.compile(
    r'依赖|上游|下游|导致|影响|关联|连接|关系|调用|引用|'
    r'depends?|causes?|affects?|linked|related|calls?|imports?|'
    r'who|whom|which|where.*from|owner|author|creator', re.IGNORECASE
)


def classify_intent(query: str) -> str:
    """Classify query intent: exact, semantic, relational, or auto.

    Rules-first approach (no LLM needed):
      - Contains error codes / function names / config → exact
      - Contains relationship words → relational
      - Otherwise → auto (balanced)
    """
    if _EXACT_PATTERNS.search(query):
        return "exact"
    if _RELATIONAL_PATTERNS.search(query):
        return "relational"
    # Short queries with known entities → semantic
    entities = _ENTITY.findall(query)
    if len(query.split()) <= 4 and entities:
        return "semantic"
    return "auto"


def get_intent_weights(query: str, mode: str = "auto") -> Dict[str, float]:
    """Get fusion weights based on query intent."""
    if mode != "auto":
        return _INTENT_WEIGHTS.get(mode, _INTENT_WEIGHTS["auto"])
    intent = classify_intent(query)
    return _INTENT_WEIGHTS[intent]
...
def normalize_scores(scored_items: List[Tuple[Any, float]]) -> List[Tuple[Any, float]]:
    ...
    if max_s == min_s:
        return [(item, 1.0) for item, _ in scored_items]
    return [(item, (s - min_s) / (max_s - min_s)) for item, s in scored_items]
...
class EnhancedSearch:
    ...
    def search(self, query: str, user_id: str = "default",
        ...
        # Fact retrieval: 4-way fusion with structured facts
        try:
            from .facts import FactStore
            fs = FactStore(self.nc._conn())
            fact_results = fs.search_text(query, limit=5)
            for fr in fact_results:
                # Synthesize a result dict compatible with the pipeline
                fact_entry = {
                    "id": f"fact_{fr['id']}",
                    "content": f"{fr['subject']} {fr['predicate']} = {fr['object']}",
                    "score": fr.get("confidence", 0.8),
                    "similarity": fr.get("confidence", 0.8),
                    "_source": "fact",
                    "layer": "consolidated",
                }
                fid = fact_entry["id"]
                if fid not in seen_results:
                    seen_results[fid] = fact_entry
                    all_results.append(fact_entry)
        except Exception:
            pass
        ...
        # Retention-aware re-rank: blend retrieval_priority into final score
        try:
            from .belief import retrieval_priority, freshness_label
            for r in all_results:
                rp = retrieval_priority(r)
                original = r.get("rerank_score") or r.get("similarity") or r.get("score") or 0
                # 80% original relevance + 20% retention priority
                r["_final_score"] = 0.8 * original + 0.2 * rp
                # Attach freshness label for context builder
                vf = r.get("valid_from") or r.get("created_at")
                vt = r.get("valid_to")
                r["_freshness_label"] = freshness_label(vf, vt) if vf else "unknown"
            all_results.sort(key=lambda x: -x.get("_final_score", 0))
        except Exception:
            pass
...
def build_context_v2(results: List[Dict],
    ...
    for r in unique:
        ...
        label = r.get("_freshness_label", "")
        if label and label not in ("today", "yesterday", "unknown"):
            tag = f" [{label}]"
        else:
            tag = ""
        parts.append(f"[{len(parts)+1}]{tag} {content}")
        char_count += est
